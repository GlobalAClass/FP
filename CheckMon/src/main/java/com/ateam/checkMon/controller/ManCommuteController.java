...
public class ManCommuteController {
	...
	public ModelAndView dayCommuteList(@RequestParam(value = "cp", defaultValue = "1") int cp, HttpSession session,
			EmpCommuteDTO dto) {
		...
		int slist_len = slist.size();
		for (int j = 0; j < slist_len; j++) {
			// 당일 스케줄 근무 날짜
			String s_workday = slist.get(j).getS_year() + "-" + slist.get(j).getS_month() + "-"
					+ slist.get(j).getS_day();
			// 스케줄 출근시간
			String starttime_s = slist.get(j).getS_start_time().replace(":", "");
			int starttime = Integer.parseInt(starttime_s);
			// 스케줄 퇴근시간
			String endtime_s = slist.get(j).getS_end_time().replace(":", "");
			int endtime = Integer.parseInt(endtime_s);
			int s_emp_ix = slist.get(j).getEmp_ix();
			boolean eq = false;
			...
			for (int i = 0; i < list.size(); i++) {
				...
				int l_emp_ix = list.get(i).getEmp_ix();
				
				if (s_emp_ix == l_emp_ix ) {
					eq = true;
					// 근무자 출근시간
					String worktime_s = list.get(i).getWorktime().replace(":", "");
					if (worktime_s.equals("-")) {
						worktime_s = "0";
					}
			...
			if(eq==false) {
				//insert emp_ix,workday,state 하기
				dto.setEmp_ix(s_emp_ix);
				if (endtime < todaytime) {
					String temp = "결근";
					dto.setState(temp);
					mdao.insertWorkdayState(dto);
				} else {
					String temp = "근무예정";
					dto.setState(temp);
					mdao.insertWorkdayState(dto);
		...
		String paging = com.ateam.checkMon.page.PageModule.getMakePage("dayCommuteList.do", totalcnt, listsize,
				pagesize, cp);
		list = mdao.getDayCommuteList(listsize, cp, man_ix);
	...
	public ModelAndView commuteApplyAllList(@RequestParam(value = "cp", defaultValue = "1") int cp,
		...
		String paging = com.ateam.checkMon.page.PageModule.getMakePage("commuteApplyAllList.do", totalcnt, listsize,
				pagesize, cp);
	...
	public ModelAndView commuteApplyReturn(EmpCommuteApplyDTO adto,
			@RequestParam(value = "c_return", required = false) String c_return, EmpCommuteDTO cdto) {
		ModelAndView mav = new ModelAndView();
